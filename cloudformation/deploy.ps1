...
acchist_Out_File    =$acchist_Out_File
file_Type			=$file_Type
cfx_Bci_Bucket      =$cfx_Bci_Bucket
cfx_Bci_Folder		=$cfx_Bci_Folder
...
echo "##### copying .whl, .zip & .py files to s3 path #####"
# 1. upload artifacts
...
echo "##### files upload to s3 complete #####"

echo "##### Glue jobs deployment begin #####"
...
aws --profile $account_Name cloudformation deploy `
    ...
    dbConnection=$db_Connection tagEnvironment=$tag_Environment nameSuffix=$name_Suffix fileNameEnvironment=$file_Name_Environment `
    fileType=$file_Type secretManager=$secret_Manager redshiftGlueArn=$redshift_Glue_Arn deploymentEnvironment=$deployment_Environment tagRegion=$tag_Region `
...
echo "##### Lambda layer deployment begin #####"
...
echo "##### Latest lambda layer in $layer_Name is $layer_With_Version_Arn #####"

echo "##### Lambda functions deployment begin #####"
...
aws --profile $account_Name cloudformation deploy `
    ...
    snsArn=$sns_Arn tagEnvironment=$tag_Environment nameSuffix=$name_Suffix secretManager=$secret_Manager fileNameEnvironment=$file_Name_Environment `
    deploymentEnvironment=$deployment_Environment acchistOutFile=$acchist_Out_File cfxBciBucket=$cfx_Bci_Bucket cfxBciFolder=$cfx_Bci_Folder `
    cfxCvsBucket=$cfx_Cvs_Bucket skipDelivery=$skip_Delivery fileType=$file_Type securityGroups=$security_Groups subnetIds=$subnet_Ids tagRegion=$tag_Region `
###### step 1 ends ######
...
echo "##### creating change set to add s3 bucket now #####"
...
aws --profile $account_Name cloudformation create-change-set `
    ...
    ParameterKey=snsArn,ParameterValue=$sns_Arn ParameterKey=tagEnvironment,ParameterValue=$tag_Environment ParameterKey=nameSuffix,ParameterValue=$name_Suffix `
    ParameterKey=fileNameEnvironment,ParameterValue=$file_Name_Environment ParameterKey=deploymentEnvironment,ParameterValue=$deployment_Environment `
    ParameterKey=acchistOutFile,ParameterValue=$acchist_Out_File ParameterKey=cfxBciBucket,ParameterValue=$cfx_Bci_Bucket `
    ParameterKey=cfxBciFolder,ParameterValue=$cfx_Bci_Folder ParameterKey=cfxCvsBucket,ParameterValue=$cfx_Cvs_Bucket `
    ParameterKey=skipDelivery,ParameterValue=$skip_Delivery ParameterKey=fileType,ParameterValue=$file_Type ParameterKey=secretManager,ParameterValue=$secret_Manager `
    ...
    ParameterKey=tagRegion,ParameterValue=$tag_Region `
...
echo "##### Lambda functions deployment and addition of triggers end #####"

echo "##### SNS topics deployment begin #####"
...
aws --profile $account_Name cloudformation deploy `
    ...
    --parameter-overrides topicName1=$topic_Name1 topicName2=$topic_Name2 topicName3=$topic_Name3 topicName4=$topic_Name4 endpointEmail=$endpoint_Email
	
echo "##### if above SNS stack updation is failed, ignore the error as there might be an already created SNS topic manually in the environment #####"
echo "##### SNS topics deployment end #####"
