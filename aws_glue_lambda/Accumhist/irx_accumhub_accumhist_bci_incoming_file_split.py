...
def lambda_handler(event, context):
    ...
    position_file = s3.get_object(Bucket=s3_bucket_name, Key=os.environ['INBOUND_FILE_SPECIFICATION'])
    ...
    header_file = TextFiles(all_lines[HEADER], HEADER, s3_bucket_name,os.environ['OUTBOUND_HEADER_PATH'], os.environ['OUTBOUND_HEADER_FILE_NAME'])
    detail_file = TextFiles(all_lines[DETAIL], DETAIL, s3_bucket_name,os.environ['OUTBOUND_DETAIL_PATH'], os.environ['OUTBOUND_DETAIL_FILE_NAME'])
    trailer_file = TextFiles(all_lines[TRAILER], TRAILER,s3_bucket_name, os.environ['OUTBOUND_TRAILER_PATH'], os.environ['OUTBOUND_TRAILER_FILE_NAME'])
    ...
    incoming_text = s3.get_object(Bucket=os.environ['INBOUND_BUCKET_NAME'], Key=file_to_be_processed)
...
def write_file(the_content, file_type_object):
    ...
    for line in range(start_line, end_line):
        ...
        for this_range in range(0,len(reading_positions)):
            write_line.append(read_line[reading_positions[this_range][READ_START]:reading_positions[this_range][READ_END]])
    ...
    s3_client.put_object(Body=csvio.getvalue(), ContentType='text/plain', Bucket=file_type_object.get_bucketname(), Key=file_name) 
    csvio.close()
...
def return_positions(line):
    ...
    print("Total Elements in this lines are " + str(len(range_string)))
...
def get_first_file_path(bucket_name, bucket_path):
    ...
    for s3_objects in bucket.objects.filter(Prefix=bucket_path):
        if s3_objects.get()['ContentType'] == 'application/x-directory':
            print('Directory is :' + s3_objects.key)
        else:
...
def move_file( source_bucket_name,  source_file_name_with_path, destination_bucket_name,destination_file_name_with_path):
   s3_resource = boto3.resource('s3')
   s3_resource.Object(destination_bucket_name, destination_file_name_with_path).copy_from(CopySource = {'Bucket': source_bucket_name, 'Key': source_file_name_with_path})
...
# *******This is a TextFiles class and it has the following attributes 
