...
if job_run:
    ...
    try:
        ...
        f_msg = " Unable to connect to Redshift while processing the glue job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Addr Glue job " + job_name + " failed"
    ...
    try:
        logger.info(" Reading Stay Addr Data from JSON ")
        ...
        f_msg = "  Error while reading Stay Addr data from S3 for the job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Addr Glue job " + job_name + " failed"
        notifymsg(f_sub, f_msg)
        raise SystemExit(e)
    ...
    if stay_df_count == 0:
        logger.info(
            'No Data to process for Stay Addr for {} run'.format(str(datetime.now())))
        f_msg = "No Data to Process for Stay Addr Glue Job - {} for run {}".format(job_name,
                                                                                           str(datetime.now()))
        f_sub = "No Data to Process for Stay Addr Glue Job - {}".format(job_name)
        notifymsg(f_sub, f_msg)
        # exit(0)
    else:
        ...
        try:
            ...
            if seq_id_df.count() == 0:
                logger.info(f"No Seq file data present to process Stay Addr job %s for run %s",
                            job_name, str(datetime.now()))
                msg = f"No Seq file data present to process Stay Addr Job {job_name} for run " \
                      f"{str(datetime.now())}"
                sub = f"No Seq file data to Process Stay Addr Job {job_name}"
                notifymsg(sub, msg)

            else:
                est_now = create_timestamp_est()
                stg_sql = (f""" SELECT st.stay_file_cnsmr_id, st.bus_nm, st.addr_typ, st.addr_ln_1, 
                            st.addr_ln_2, st.addr_ln_3, st.addr_ln_4, st.cty_nm, st.st_cd, st.post_cd,
							st.cntry_cd, st.lat, st.long, cast(sq.job_run_id as bigint) as btch_num,
							st.data_src_nm, st.src_file_nm, st.cur_rec_ind, st.job_sess_id, 
            ...
            logger.error(
                "************ {} [ERROR] Exception while doing Data Transformtion on Stay Addr Data *************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error:  {} **************".format(str(e)))
            f_msg = "  Error while doing Data Transformtion on Stay Addr Data for Glue Job {0} , failed with error: {1}  ".format(
                    job_name, str(e))
            f_sub = "Stay Addr Glue job " + job_name + " failed"
            notifymsg(f_sub, f_msg)
            raise SystemExit(e)
    ...
    #################### Dataload to Redshift ########################
        ...
        try:
            ...
            logger.info(" Data Load Process for Stay Addr to redshift complete")
        except Exception as e:
            logger.error(str(e))
            logger.info(
        ...
        try:
            ...
            logger.error(
                "************ {} [ERROR] Exception while writing the marker file to S3 for Stay Addr Glue Job************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error: {} **************".format(str(e)))
            f_msg = "  Error while writing the marker file to S3 for Stay Addr Glue Job {}, failed with Error: {}".format(
                job_name, str(e))
...
logger.info(
    " ************** {} End of Load process for Stay Addr *********************** ".format(
        str(datetime.now())))
job.commit()
