...
if job_run:
    ...
    try:
        ...
        f_msg = " Unable to connect to Redshift while processing the glue job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Brand Rstrc Glue job " + job_name + " failed"
    ...
    try:
        logger.info(" Reading Stay Brand Rstrc Data from JSON ")
        ...
        f_msg = "  Error while reading Stay Brand Rstrc data from S3 for the job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Brand Rstrc Glue job " + job_name + " failed"
        notifymsg(f_sub, f_msg)
        raise SystemExit(e)
    ...
    if stay_df_count == 0:
        logger.info(
            'No Data to process for Stay Brand Rstrc for {} run'.format(str(datetime.now())))
        f_msg = "No Data to Process for Stay Brand Rstrc Glue Job - {} for run {}".format(job_name,
                                                                                           str(datetime.now()))
        f_sub = "No Data to Process for Stay Brand Rstrc Glue Job - {}".format(job_name)
        notifymsg(f_sub, f_msg)
        # exit(0)
    else:
        ...
        try:
            ...
            if seq_id_df.count() == 0:
                logger.info(f"No Seq file data present to process Stay Brand Rstrc job %s for run %s",
                            job_name, str(datetime.now()))
                msg = f"No Seq file data present to process Stay Brand Rstrc Job {job_name} for run " \
                      f"{str(datetime.now())}"
                sub = f"No Seq file data to Process Stay Brand Rstrc Job {job_name}"
                notifymsg(sub, msg)

            else:
                est_now = create_timestamp_est()
                
                opt_in_dt_val = datetime.now().strftime('%Y%m%d')
                
                stg_sql = (f""" SELECT st.stay_file_cnsmr_id, st.data_src_id, st.bus_unt_cd, st.brand_id,
                            st.rstrc_typ, st.rstrc_lvl, st.opt_in_dt, cast(sq.job_run_id as bigint) as btch_num, 
                            st.data_src_nm, st.src_file_nm, st.cur_rec_ind, st.job_sess_id, sq.job_run_id,
                            st.usr_cmnt, 'Glue' as create_by,
                ...
                stg = stg.withColumn("opt_in_dt",lit(opt_in_dt_val))
                stg.show()
            ...
            logger.error(
                "************ {} [ERROR] Exception while doing Data Transformtion on Stay Brand Rstrc Data *************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error:  {} **************".format(str(e)))
            f_msg = "  Error while doing Data Transformtion on Stay Brand Rstrc Data for Glue Job {0} , failed with error: {1}  ".format(
                    job_name, str(e))
            f_sub = "Stay Brand Rstrc Glue job " + job_name + " failed"
            notifymsg(f_sub, f_msg)
            raise SystemExit(e)
    ...
    #################### Dataload to Redshift ########################
        ...
        try:
            ...
            logger.info(" Data Load Process for Stay Brand Rstrc to redshift complete")
        except Exception as e:
            logger.error(str(e))
            logger.info(
        ...
        try:
            ...
            logger.error(
                "************ {} [ERROR] Exception while writing the marker file to S3 for Stay Brand Rstrc Glue Job************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error: {} **************".format(str(e)))
            f_msg = "  Error while writing the marker file to S3 for Stay Brand Rstrc Glue Job {}, failed with Error: {}".format(
                job_name, str(e))
...
logger.info(
    " ************** {} End of Load process for Stay Brand Rstrc *********************** ".format(
        str(datetime.now())))
job.commit()
