...
if job_run:
    ...
    try:
        ...
        f_msg = " Unable to connect to Redshift while processing the glue job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Jrnl Glue job " + job_name + " failed"
    ...
    try:
        logger.info(" Reading Stay Jrnl Data from JSON ")
        ...
        f_msg = "  Error while reading Stay Jrnl data from S3 for the job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Jrnl Glue job " + job_name + " failed"
        notifymsg(f_sub, f_msg)
        raise SystemExit(e)
    ...
    if stay_df_count == 0:
        logger.info(
            'No Data to process for Stay Jrnl for {} run'.format(str(datetime.now())))
        f_msg = "No Data to Process for Stay Jrnl Glue Job - {} for run {}".format(job_name,
                                                                                           str(datetime.now()))
        f_sub = "No Data to Process for Stay Jrnl Glue Job - {}".format(job_name)
        notifymsg(f_sub, f_msg)
        # exit(0)
    else:
        ...
        try:
            ...
            if seq_id_df.count() == 0:
                logger.info(
                    f"No Seq file data present to process Stay Jrnl job %s for run %s",
                    job_name, str(datetime.now()))
                msg = f"No Seq file data present to process Stay Jrnl Job {job_name} for " \
                      f"run " \
                      f"{str(datetime.now())}"
                sub = f"No Seq file data to Process Stay Jrnl Job {job_name}"
                notifymsg(sub, msg)

            else:
                est_now = create_timestamp_est()
                stg_sql = (f""" SELECT st.stay_file_cnsmr_id, st.jrnl_acct_typ, st.jrnl_dt, st.jrnl_tm,
                            st.evnt_ref_num, st.rm_num, st.frst_nm, st.mid_nm, st.lst_nm, st.trans_cd,
                            st.trans_desc, cast(st.trans_amt as decimal(20,5)), st.cmdy_cd, st.adj_rsn_cd,
                            st.adj_rsn_desc, st.adj_allow_flg, cast(st.jrnl_amt as decimal(20,5)), 
                            cast(st.pmt_amt as decimal(20,5)), cast(st.chrg_amt as decimal(20,5)), 
                            st.clrk_init, cast(sq.job_run_id as bigint) as btch_num, st.data_src_nm,
                            st.src_file_nm, st.cur_rec_ind, st.job_sess_id, sq.job_run_id, st.usr_cmnt,
            ...
            logger.error(
                "************ {} [ERROR] Exception while doing Data Transformtion on Stay Jrnl Data *************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error:  {} **************".format(str(e)))
            f_msg = "  Error while doing Data Transformtion on Stay Jrnl Data for Glue Job {0} , failed with error: {1}  ".format(
                    job_name, str(e))
            f_sub = "Stay Jrnl Glue job " + job_name + " failed"
            notifymsg(f_sub, f_msg)
            raise SystemExit(e)
    ...
    #################### Dataload to Redshift ########################
        ...
        try:
            ...
            logger.info(" Data Load Process for Stay Jrnl to redshift complete")
        except Exception as e:
            logger.error(str(e))
            logger.info(
        ...
        try:
            ...
            logger.error(
                "************ {} [ERROR] Exception while writing the marker file to S3 for Stay Jrnl Glue Job************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error: {} **************".format(str(e)))
            f_msg = "  Error while writing the marker file to S3 for Stay Jrnl Glue Job {}, failed with Error: {}".format(
                job_name, str(e))
...
logger.info(
    " ************** {} End of Load process for Stay Jrnl *********************** ".format(
        str(datetime.now())))
job.commit()
