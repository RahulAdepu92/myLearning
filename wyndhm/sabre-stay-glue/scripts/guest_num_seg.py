...
if job_run:
    ...
    try:
        ...
        f_msg = " Unable to connect to Redshift while processing the glue job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Guest Num Seg Glue job " + job_name + " failed"
    ...
    try:
        logger.info(" Reading Stay Guest Num Seg Data from JSON ")
        stay_s3_read_dyf = glueContext.create_dynamic_frame.from_options(connection_type="s3",
                                                                        ...
                                                                        format="json",
                                                                        format_options={
                                                                            "jsonPath": "$.stg_stay_guest_num_seg[*]"})
        ...
        f_msg = "  Error while reading Stay Guest Num Seg data from S3 for the job {0} , failed with error: {1}  ".format(
            job_name, str(e))
        f_sub = "Stay Guest Num Seg Glue job " + job_name + " failed"
        notifymsg(f_sub, f_msg)
        raise SystemExit(e)
    ...
    if stay_df_count == 0:
        logger.info(
            'No Data to process for Stay Guest Num Seg for {} run'.format(str(datetime.now())))
        f_msg = "No Data to Process for Stay Guest Num Seg Glue Job - {} for run {}".format(job_name,str(datetime.now()))
        f_sub = "No Data to Process for Stay Guest Num Seg Glue Job - {}".format(job_name)
        notifymsg(f_sub, f_msg)
        # exit(0)
    else:
        ...
        try:
            ...
            if seq_id_df.count() == 0:
                logger.info(
                    f"No Seq file data present to process Stay Guest Num Seg job %s for run %s",
                    job_name, str(datetime.now()))
                msg = f"No Seq file data present to process Stay Guest Num Seg Job {job_name} for " \
                      f"run " \
                      f"{str(datetime.now())}"
                sub = f"No Seq file data to Process Stay Guest Num Seg Job {job_name}"
                notifymsg(sub, msg)

            else:
                est_now = create_timestamp_est()
                stg_sql = (f""" SELECT st.stay_file_cnsmr_id, st.num_seg_typ, st.num_seg_cd, st.num_seg_val,
                            st.mbr_enrl_dt, st.num_exp_dt, st.affinity_crd_ind, st.num_seg_sts, 
                            st.num_seg_sts_dt, cast(sq.job_run_id as bigint) as btch_num,
            ...
            logger.error(
                "************ {} [ERROR] Exception while doing Data Transformtion on Stay Guest Num Seg Data *************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error:  {} **************".format(str(e)))
            f_msg = "  Error while doing Data Transformtion on Stay Guest Num Seg Data for Glue Job {0} , failed with error: {1}  ".format(
                    job_name, str(e))
            f_sub = "Stay Guest Num Seg Glue job " + job_name + " failed"
            notifymsg(f_sub, f_msg)
            raise SystemExit(e)
    ...
    #################### Dataload to Redshift ########################
        ...
        try:
            ...
            logger.info(" Data Load Process for Stay Guest Num Seg to redshift complete")
        except Exception as e:
            logger.error(str(e))
            logger.info(
        ...
        try:
            ...
            logger.error(
                "************ {} [ERROR] Exception while writing the marker file to S3 for Stay Guest Num Seg Glue Job************".format(
                    str(datetime.now())))
            logger.error(
                "*********** [ERROR] Failing with error: {} **************".format(str(e)))
            f_msg = "  Error while writing the marker file to S3 for Stay Guest Num Seg Glue Job {}, failed with Error: {}".format(
                job_name, str(e))
...
logger.info(
    " ************** {} End of Load process for Stay Guest Num Seg *********************** ".format(
        str(datetime.now())))
job.commit()
