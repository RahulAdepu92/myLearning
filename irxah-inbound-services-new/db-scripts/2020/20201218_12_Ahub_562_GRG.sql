-- delete from ahub_dw.export_setting where client_file_id=17;

-- insert queries for Gila River
...
SELECT 
    ...
    (select top 1 output_sender_id from ahub_dw.client_files where client_file_id=17) AS sender_identifier,
    (select top 1 output_receiver_id from ahub_dw.client_files where client_file_id=17) AS receiver_identifier,
  ...
  (
    ...
    (
      ...
      where j.job_key in 
        ...
        and j.sender_identifier = (select top 1 input_sender_id from ahub_dw.client_files where client_file_id=17)
        and transmission_file_type =''DQ''
        and transaction_response_status = ''''
        and client_pass_through = ''INGENIORXGILA00489''
    )z
  )
  where RowNumber = 1
