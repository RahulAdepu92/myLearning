-- delete from ahub_dw.export_setting where client_file_id=21;

-- insert queries for The Health Plan error file
...
SELECT
    ...
    record_type,dr_transmission_file_type,version_release_number,
    (select top 1 output_sender_id from ahub_dw.client_files where client_file_id=21) AS sender_identifier,
    (select top 1 output_receiver_id from ahub_dw.client_files where client_file_id=21) AS receiver_identifier,
    submission_number,dr_transaction_response_status,dr_reject_code,
  ...
  FROM (
    ...
    WHERE 
        ...
        AND acc.sender_identifier = (select top 1 input_sender_id from ahub_dw.client_files where client_file_id=21)
        AND transmission_file_type = ''DR''
        AND transaction_response_status = ''R''
        and client_pass_through in ( ''INGENIORXMOLDFIB00489'' )
    )
  ...
  FROM 
    (
      ...
      WHERE job_key IN (SELECT job_key
                          FROM ahub_dw.job
                          WHERE client_file_id in (18) -- file came from THP
                      )
        ...
        (
          ...
          WHERE val.validation_result = ''N''
                ...
                val.job_key IN (
                    ...
                    WHERE client_file_id in (18) -- came from THP
                    )
        )
      )
