/* this whole script intends to introduce new Melton-GPA file with
client_file_id = 14 (GPA → Database) and client_file_id = 15 (Database → GPA) */

------ ################## Insert client_file_id = 14  ( GPA Melton --> AHUB ) ##################
...
---below values are changed according to the incoming/outgoing file requirements
  ...
  300,
  true,
  'GPA Melton Accumulation File',
  'Process: Incoming file from GPA Melton is stored in database
Path : GPA Melton  to  Database
File Pattern : ACCDLYINT_TST_GPAINRX_YYMMDDHHMMSS (UAT), ACCDLYINT_PRD_GPAINRX_YYMMDDHHMMSS (PRD)
Frequency : Daily (Except Saturday and Sunday)
Time : 6:30am and 9:00am CST',
  True,
  'inbound/archive',
  'inbound/error',
  'inbound/specifications/gpa_melton_structure_specification.csv',
  1700,
  '',
  '',
  '20700GPA',
  ...
  'GPA Melton file to database',
  'Validate inbound GPA Melton file, split, load into redshift database',
  NULL,
  true,
  true,
  false,
...
---below values are initially inserted taking SIT as reference. They should be updated according to environment while running UPDATE statement in next step
  'ACCDLYINT_TST_GPAINRX_YYMMDDHHMMSS',
  'SIT',
  '',
  '',
...
------------- Step 2 -------------
-- RUN below UPDATE statements as per environment (UAT/PROD) for client_file_id = 14  ( GPA Melton --> AHUB )
...
-- Following values would change per environment
       ...
       name_pattern = 'ACCDLYINT_TST_GPAINRX_YYMMDDHHMMSS',
       deliver_files_to_client = False
       WHERE client_file_id IN (14);
...
-- Following values would change per environment
       ...
       name_pattern = 'ACCDLYINT_PRD_GPAINRX_YYMMDDHHMMSS',
       deliver_files_to_client = True
       WHERE client_file_id IN (14);


------------- Step 3  -------------
-- Insert file columns records in the ahub_dw.file_columns for client_file_id = 14  ( GPA Melton --> AHUB )
-- Here we are copying the columnar metadata of client_file_id=1 to the client_file_id=14
...
------------- Step 4 -------------
-- Insert column rules records in the ahub_dw.column_rules  for client_file_id = 14  ( GPA Melton --> AHUB )
...
(SELECT (cr.column_rules_id +14000),
       (fc.file_column_id +14000),
       cr.priority,
       cr.validation_type,
       cr.equal_to,
       cr.python_formula,
...
------------- Step 5 -------------
-- UPDATE records in 'ahub_dw.column_rules' for client_file_id = 14  ( GPA Melton --> AHUB )
...
---Updating Client Pass Through value for Gpa in column rules table
...
FROM ahub_dw.file_columns
WHERE client_file_id = 14
AND file_column_name = 'Client Pass Through');

--- Updating Sender ID value for Gpa in column rules table
UPDATE ahub_dw.column_rules
SET equal_to = '20700GPA'
WHERE validation_type = 'EQUALTO'
AND file_column_id IN (SELECT file_column_id
FROM ahub_dw.file_columns
WHERE client_file_id = 14
...
--- Gpa supports 12 accumulators. Hence making 5-12 accumulators (which are imported of BCI will have 'N') active ( is_active = 'Y' ).
UPDATE ahub_dw.column_rules
SET is_active = 'Y'
...
---update list_of_values to validate upto 12 accumulators for GPA
UPDATE ahub_dw.column_rules
...
------- ################## Insert client_file_id = 15  ( AHUB --> GPA Melton ) ##################
...
---below values are changed according to the incoming/outgoing file requirements
  ...
  true,
  'GPA Melton Accumulation File',
  'Process: From database Outbound file is generated and exported to GPA Melton
Path : Database to GPA Melton
File Pattern : ACCDLYINT_TST_INRXGPA_YYMMDDHHMMSS (UAT)
ACCDLYINT_PRD_INRXGPA_YYMMDDHHMMSS (PRD)
Frequency : Daily
Time : 05:00 AM - 07:00 AM EST',
  ...
  'outbound/temp',
  '',
  'Database to Gpa Melton file',
  'From database export outbound file to GPA Melton',
  NULL,
  false,
  false,
  false,
  '',
---below values are initially inserted taking SIT as reference. They should be updated according to environment while running UPDATE statement in next step
  'ACCDLYINT_TST_INRXGPA_YYMMDDHHMMSS',
  'SIT',
  'ACCDLYINT_TST_INRXGPA_',
  'T',
---below values remain constant for any incoming file. Need not overwrite them.
  'AHUB',
  NULL,
...
---below values are for lambda functions and vary as per environment. For example, delivers_to_client is FALSE in lower envs and TRUE in higher envs.
  ...
  False,
  False,
  'irx-nonprod-melton-gpa-east-1-sftp-app-outbound',
  'sit/accums/',
  '20MB',
  '01/01/2021'
);
...
------------- Step 2 -------------
-- RUN below UPDATE statements as per environment (UAT/PROD) for client_file_id = 15  (  AHUB --> GPA Melton )
-- By this we are updating the selective columns that satisfy the environmental level requirements.
-- For example: name_pattern varies between UAT and PROD.
...
-- Following values would change per environment
       environment_level = 'PRE-PROD',
       name_pattern = 'ACCDLYINT_TST_INRXGPA_YYMMDDHHMMSS',
       s3_output_file_name_prefix = 'ACCDLYINT_TST_INRXGPA_',
       outbound_file_type_column = 'T',
       destination_bucket_cfx = 'irx-prod-melton-gpa-east-1-sftp-app-outbound',
       destination_folder_cfx = 'preprod/accums/',
       deliver_files_to_client = False
       WHERE client_file_id IN (15);
...
-- Following values would change per environment
       environment_level = 'PROD',
       name_pattern = 'ACCDLYINT_PRD_INRXGPA_YYMMDDHHMMSS',
       s3_output_file_name_prefix = 'ACCDLYINT_PRD_INRXGPA_',
